...
def oauth_login(q):
    ...
    return redirect(settings.YASOI['authorize_url'] + '?' + urlencode(dict(
        response_type='code',
...
def oauth_auth(q):
    ...
    code = q.GET['code']
    q2 = requests.post(settings.YASOI['token_url'], data=dict(
    ...
    q3 = requests.get(settings.YASOI['me_url'], headers={'Authorization': f'Bearer {access_token}'})
    q3.raise_for_status()
    s3d = q3.json()
    try:
